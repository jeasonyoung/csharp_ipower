...
namespace iPower.Cryptography
{
    ...
    public static class XXTEA
    {
        ...
        {
            return HexParser.ToHexString(Encrypt(Encoding.UTF8.GetBytes(data), HashCrypto.Hash(Encoding.UTF8.GetBytes(key), "md5")));
        }
        ...
        {
            return Encoding.UTF8.GetString(Decrypt(HexParser.Parse(data), HashCrypto.Hash(Encoding.UTF8.GetBytes(key), "md5")));
        }
        #endregion

        #region 辅助函数。
        static uint[] ToUInt32Array(byte[] data, bool includeLength)
        {
        ...
        static byte[] ToByteArray(uint[] data, bool includeLength)
        {
            int n = data.Length << 2;
            if (includeLength)
            {
                ...
                if (m > n)
                {
                    throw new CryptographyException("XXTEA Decrypt Error: Wrong input data.");
                }
                else
                {
                    n = m;
